...
export function appendDigitLedgerLimit(buffer, digit) {
...
export function deleteLastDigitLedgerLimit(buffer) {
  return String(buffer || '').slice(0, -1);
}

export function bufferToLedgerLimit(buffer) {
...
function mkId(op, arg = '') {
  if (op === 'd') return `${BOTING_LEDGER_LIM_KPAD_PREFIX}|d|${arg}`;
  return `${BOTING_LEDGER_LIM_KPAD_PREFIX}|${op}`;
}

/**
 * @param {{ buffer: string, extraFlags?: number }} opts
 */
export function buildBotingLedgerLimitKeypadPayload({ buffer, extraFlags = 0 }) {
  const bid = mkId;
 ...
 */
export function parseBotingLedgerLimitKeypadId(customId) {
  const p = String(customId).split('|');
  if (p[0] !== BOTING_LEDGER_LIM_KPAD_PREFIX || p.length < 2) return null;
